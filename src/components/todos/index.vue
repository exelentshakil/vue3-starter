...
<div class="page-content page-container" id="page-content">
    <div class="padding d-flex justify-content-center">
        <div class="row container d-flex justify-center-center">
            <div class="col-md-12">
                <div class="card px-3">
                    <div class="card-body">
                        <h4 class="card-title">{{ title }}</h4><button class="add btn btn-primary font-weight-bold todo-list-add-btn mb-2" @click="deleteAll">Delete All</button>
                        <div class="add-items d-flex"> <input type="text" class="form-control todo-list-input" placeholder="What do you need to do today?" :value="todoTitle" @keyup="setTitle"> <button class="add btn btn-primary font-weight-bold todo-list-add-btn" @click="addTodo">Add</button> </div>
                        <div class="list-wrapper">
                            <ul class="d-flex flex-column-reverse todo-list">
                                <li v-for="todo in todos" v-bind:key="todo.id" :class="{completed : todo.completed}">
                                    <div class="form-check">
                                        <label class="form-check-label"> <input class="checkbox" type="checkbox" @change="markCompleted" :value="todo.id"> {{ todo.title }}<i class="input-helper"></i></label> </div> <i class="remove mdi mdi-close-circle-outline" @click="deleteTodo(todo.id)"></i>
                                </li>
                            </ul>
                        </div>
                    </div>
...
export default {
    ...
    setup() {
        ...
        const addTodo = async () => {
            ...
            todos.value.push({id: response.data.id, title: response.data.params.title, completed:false});
        }

        const markCompleted = (e) => {
            todos.value.map( todo => todo.id == e.target.value ? todo.completed = !todo.completed : todo);
        }
